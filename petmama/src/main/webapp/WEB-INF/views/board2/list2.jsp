...
<title>후기 목록</title>
<link rel="stylesheet"
	href="${pageContext.request.contextPath}/css/customer/board2/style.css">
...
<body>
	...
	<div class="container-fluid bg-info bg-opacity-50"
		...
		<br>
		<br>
		<h5 class="text-center text-white text-center">PetMama 펫시터는
			어떠셨나요?</h5>
	</div>
	<br>
	<br>
	<div class="page-main">
		<div class="content-main">
			<div class="container">
				<form id="search_form" action="list2.do" method="get">
					<!-- 검색을 제외하고 데이터베이스를 전송할땐 post방식 써야함 -->
					<ul
						class="search search d-flex justify-content-center list-unstyled">
						<li><select name="keyfield">
								<option value="1"
									<c:if test="${param.keyfield==1}">selected</c:if>>제목</option>
								<option value="2"
									<c:if test="${param.keyfield==2}">selected</c:if>>작성자</option>
								<option value="3"
						...
						<tbody>
							<c:forEach var="board1" items="${list}">
								<tr>
									...
									<td><c:if test="${!empty board1.mem_nickname}">${board1.mem_nickname}</c:if>
					...
					<div class="list-space d-flex mb-3">
						<div class="me-auto p-2">
							<input type="button" value="글쓰기"
								onclick="location.href='writeForm2.do'"
