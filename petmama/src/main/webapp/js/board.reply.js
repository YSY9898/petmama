$(function(){
		...
		$.ajax({
			...
			success:function(param){
				...
				$(param.list).each(function(index,item){
					...
					output += '<h6><b>' + item.mem_nickname + '</b></h6>';
					...
					if(param.user_num == item.mem_num){//로그인한 회원번호와 작성자 회원번호 일치
						output += ' <input type="button" data-renum="' + item.re_num + '" value="수정" class="modify-btn list-btn">';
						output += ' <input type="button" data-renum="' + item.re_num + '" value="삭제" class="delete-btn list-btn">';
					}
	...
	$(document).on('click','.modify-btn',function(){
		...
		modifyUI += '<textarea rows="3" cols="80" name="re_content" id="mre_content" class="rep-content">' + content + '</textarea>';
		modifyUI += '<div id="mre_first"><span class="letter-count">300/300</span></div>';
		modifyUI += '<div id="mre_second" class="align-right">';
		modifyUI += ' <input type="submit" value="수정" class="list-btn">';
		modifyUI += ' <input type="button" value="취소" class="re-reset list-btn">';
		modifyUI += '</div>';
